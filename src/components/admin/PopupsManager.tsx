...
function PopupForm({ initialData, onClose, onSaved }: { initialData?: PopupRecord, onClose: () => void, onSaved: () => void }) {
...
const PopupsManager = forwardRef<PopupsManagerHandle>((_props, ref) => {
  ...
  const handleDelete = async (id: number) => {
    const result = await Swal.fire({
      title: 'ยืนยัน?',
      text: 'ยืนยันการลบป๊อปอัปนี้หรือไม่?',
      icon: 'warning',
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        title={editingPopup ? 'แก้ไขป๊อปอัป' : 'สร้างป๊อปอัปใหม่'}
        maxWidth="max-w-3xl"
      >
        <PopupForm
