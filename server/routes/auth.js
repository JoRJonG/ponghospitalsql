...
router.post('/login', authLimiter, validate(loginSchema), async (req, res) => {
  ...
  const sessionCookieOptions = {
    httpOnly: true,
    sameSite: isProduction ? 'strict' : 'lax', // OWASP: Strict for session cookies
    secure: secureCookie,
    path: '/',
  ...
  const refreshCookieOptions = {
    httpOnly: true,
    sameSite: isProduction ? 'strict' : 'lax', // OWASP: Strict for refresh tokens
    secure: secureCookie,
    path: '/api/auth/refresh', // OWASP: Restrict path for refresh token
...
router.post('/logout', (req, res) => {
  try {
    ...
    res.clearCookie('ph_refresh_token', { path: '/api/auth/refresh' })
