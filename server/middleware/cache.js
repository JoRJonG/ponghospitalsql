...
export function microCache(ttlMs = 10_000, browserCacheSeconds = 0) {
  return function (req, res, next) {
    ...
    const setCacheHeaders = (isHit) => {
      // ALWAYS force revalidation to support 304 correctly
      // We ignore browserCacheSeconds to prevent disk caching
      // Remove no-store so browser can save ETag
      res.setHeader('Cache-Control', 'no-cache, must-revalidate')
      res.setHeader('Pragma', 'no-cache')
      res.setHeader('Expires', '0')
      res.setHeader('X-Micro-Cache', isHit ? 'HIT' : 'MISS')
    }
