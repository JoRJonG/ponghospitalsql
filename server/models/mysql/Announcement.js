  ...
  static _buildWhereClause(filter) {
    ...
    if (filter.status) {
      if (filter.status === 'published') {
        whereClause += ' AND a.is_published = 1 AND (a.published_at IS NULL OR a.published_at <= NOW())'
      } else if (filter.status === 'scheduled') {
        whereClause += ' AND a.is_published = 1 AND a.published_at > NOW()'
      ...
      // Backward compatibility: isPublished=true usually implies 'now' visible
